...
module control (
    ...
    always_comb begin
        ...
        case (funct_r)
            R_ADD : ctrl_r.alu_op = ALU_ADD;
            R_SUB : ctrl_r.alu_op = ALU_SUB;
            R_AND : ctrl_r.alu_op = ALU_AND;
            R_OR : ctrl_r.alu_op = ALU_OR;
            R_XOR : ctrl_r.alu_op = ALU_XOR;
            R_SLL : ctrl_r.alu_op = ALU_SLL;
            R_SRL : ctrl_r.alu_op = ALU_SRL;
            R_SRA : ctrl_r.alu_op = ALU_SRA;
            R_SLT : ctrl_r.alu_op = ALU_SLT;
            R_SLTU : ctrl_r.alu_op = ALU_SLTU;
        endcase
    end
    ...
    always_comb begin
        ...
        case (opcode_i)
            I_ADDI : ctrl_i.alu_op = ALU_ADD;
            I_ANDI : ctrl_i.alu_op = ALU_AND;
            I_ORI : ctrl_i.alu_op = ALU_OR;
            I_XORI : ctrl_i.alu_op = ALU_XOR;
            I_SLLI : ctrl_i.alu_op = ALU_SLL;
            I_SRLI_SRAT : ctrl_i.alu_op = funct7_bit5 ? ALU_SRA : ALU_SRL;
            I_SLTI : ctrl_i.alu_op = ALU_SLT;
            I_SLTIU : ctrl_i.alu_op = ALU_SLTU;

            I_LB : {ctrl_i.mem_valid, ctrl_i.mem_size, ctrl_i.wb_src, ctrl_i.load_zero_extend} = {1'b1, MEM_BYTE, WB_SRC_MEM, 1'b0};
            I_LH : {ctrl_i.mem_valid, ctrl_i.mem_size, ctrl_i.wb_src, ctrl_i.load_zero_extend} = {1'b1, MEM_HALF, WB_SRC_MEM, 1'b0};
            I_LW : {ctrl_i.mem_valid, ctrl_i.mem_size, ctrl_i.wb_src, ctrl_i.load_zero_extend} = {1'b1, MEM_WORD, WB_SRC_MEM, 1'b0};
            I_LBU : {ctrl_i.mem_valid, ctrl_i.mem_size, ctrl_i.wb_src, ctrl_i.load_zero_extend} = {1'b1, MEM_BYTE, WB_SRC_MEM, 1'b1};
            I_LHU : {ctrl_i.mem_valid, ctrl_i.mem_size, ctrl_i.wb_src, ctrl_i.load_zero_extend} = {1'b1, MEM_HALF, WB_SRC_MEM, 1'b1};
        endcase
    end
    ...
    always_comb begin
        ...
        case (funct3)
            S_SB: ctrl_s.mem_size = MEM_BYTE;
            S_SH: ctrl_s.mem_size = MEM_HALF;
            S_SW: ctrl_s.mem_size = MEM_WORD;
        endcase
    end
